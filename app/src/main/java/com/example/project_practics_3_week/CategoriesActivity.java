...
public class CategoriesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        categoryRecyclerView = findViewById(R.id.buttons_recycler_view);
    ...
    private void fetchCategories() {
        Network.getApiService().getCategories(API_KEY, AUTH_TOKEN).enqueue(new Callback<List<Category>>() {
            @Override
            public void onResponse(Call<List<Category>> call, Response<List<Category>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    categoryAdapter.updateCategories(response.body());
                } else {
                    Toast.makeText(CategoriesActivity.this, "Не удалось загрузить категории", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<Category>> call, Throwable t) {
                Toast.makeText(CategoriesActivity.this, "Ошибка: " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void fetchProducts(String categoryId) {
        Call<List<Product>> call = categoryId == null ?
                Network.getApiService().getAllProducts(API_KEY, AUTH_TOKEN) :
                Network.getApiService().getProducts(API_KEY, AUTH_TOKEN, "eq." + categoryId);
        ...
        call.enqueue(new Callback<List<Product>>() {
            ...
            public void onResponse(Call<List<Product>> call, Response<List<Product>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    Toast.makeText(CategoriesActivity.this, "Не удалось загрузить товары", Toast.LENGTH_SHORT).show();
                }
            }
            ...
            public void onResponse(Call<List<User>> call, Response<List<User>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (updatedUser != null) {
                        ...
                        Toast.makeText(CategoriesActivity.this, "Данные синхронизированы", Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(CategoriesActivity.this, "Пользователь не найден", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(CategoriesActivity.this, "Ошибка сервера: " + response.code(), Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<User>> call, Throwable t) {
                Toast.makeText(CategoriesActivity.this, "Ошибка синхронизации: " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
